...
`../openspec/changes/session-instances/`, решение D3). Проверено 23.09.2026:
ключа `session.restore_on_startup` нет ни в одном профиле, действует
значение браузера по умолчанию, и Chromium после перезапуска открыл только
одно окно; второе окно не возвращается, но и пустым не открывается. Включать
ли настройку, решает пользователь. Признак того, что она включена, —
`session.restore_on_startup` со значением 1 в `Preferences` профиля.
