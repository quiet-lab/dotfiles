...
- Итоги проверок `windows-join-workspace` (раздел 4 его `tasks.md`): программа из лаунчера входит в `work` и ездит с ним, Super+Backspace, диспетчер задач Chrome по Shift+Esc остаётся свободным (если нет — точный заголовок окна для `dialog_title`), Ctrl+Super+S дважды, восстановление после перезагрузки.
- Итоги проверок `qs-notifications` (раздел 4 его `tasks.md`): клики по карточкам, Super+Enter с растворением, плитка над треем со счётчиком, «не беспокоить», очистка истории, клик мимо карточек в полосе столбика, Ctrl+Enter в Telegram.
- Мешает ли, что обычное окно wezterm на столе `work` теперь входит в workspace как окно семейства `wezterm` (следствие шага 3); если да — класс в `ignore_classes`.
