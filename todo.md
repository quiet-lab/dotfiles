...
- Пароль для sudo спрашивает только окно Quickshell askpass (запасной pinentry удалён, `Path askpass` в `/etc/sudo.conf`, `SUDO_ASKPASS` в сессии; оформление в общем модуле `dot_config/quickshell/common`): изменение [`askpass-only-quickshell`](openspec/changes/askpass-only-quickshell/tasks.md) реализовано, ждёт проверки пользователем (раздел 5), архивация после `work-area-center`.
...
3. Получить итоги проверок раздела 4 `live-layout` и заархивировать его после `session-instances` (`openspec archive live-layout -y`, пункты 5.1–5.2 [`tasks.md`](openspec/changes/live-layout/tasks.md)): шесть требований взяты из дельт `workspace-overrides` (в архиве) и `session-instances`; ссылки на изменение в `docs/window-model.md` сменить на архивные.
...
```bash
openspec list                              # активны session-instances (ждёт проверки перезагрузкой), keys-help (ждёт проверки), live-layout (ждёт проверки, архивация после session-instances)
