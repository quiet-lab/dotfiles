...
- [ ] Переход панели на события (22.09.2026, решение [`0007`](docs/decisions/0007-no-timers.md)) разошёлся с тремя требованиями спецификаций; свести их одним изменением OpenSpec: qs-workspaces — «переподключаться к демону не реже раза в 5 секунд» (теперь по событию обрыва и появления сокета), qs-tray-lang — «панель громкости скрывается с задержкой не более 500 мс, возврат указателя отменяет скрытие» (теперь скрывается сразу по уходу указателя с плитки), qs-monitors — «общий объём памяти обновляется раз в час» (теперь приходит с каждым ответом скрипта `ram`).
