...
- [ ] Панель не переподключается к демону после его перезапуска: 22.09.2026 после `systemctl --user restart workspaced.service` значки workspace пропали, `ss -x` показывал сокет демона без единого подключения, а таймер переподключения в `Wsd.qml` не сработал (вероятно, `Socket.connected` у Quickshell после закрытия со стороны сервера остаётся `true`, и таймер не запускается). Помогает только перезапуск `quickshell-panel.service`. По решению [`0007`](docs/decisions/0007-no-timers.md) сам таймер переподключения тоже под запретом: проверить поведение `Socket` при обрыве и переподключаться по событию ошибки или обрыва соединения, а не по таймеру, независимо от `connected`.
