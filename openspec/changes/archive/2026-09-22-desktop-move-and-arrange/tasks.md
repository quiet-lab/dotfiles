...
- [x] 3.2 Применить конфиг (`chezmoi apply --force ~/.config/workspaced/config.toml`), перезапустить `workspaced.service` с новым бинарником, проверить `workspaced check` и `workspaced keys --list` на живом конфиге. Откладывалось: прежний демон стоял на проверке у пользователя, а конфиг с действием `move` он отклонил бы
...
- [x] 5.1 На столе 2 (`surf`): сдвинуть окно мышью, нажать Ctrl+Super+3 — workspace целиком переезжает на стол 3 вместе со всеми окнами, сдвинутое окно стоит там, куда его сдвинули; Ctrl+Super+2 возвращает `surf` на стол 2; повторное Ctrl+Super+3 на столе 3 ничего не делает, а не забирает workspace соседнего стола. После нескольких переносов подряд `workspaced status --json` показывает каждый workspace ровно на одном столе, а стол, с которого workspace ушёл, — без активного (D16); Super+Tab Tab на таком столе не возвращает ушедший workspace
- [x] 5.2 На столе 1 (`work`): растащить окна мышью и нажать Ctrl+Super+Пробел — окна приложений возвращаются в свои ячейки; два окна программы, не описанной в конфиге, встают стопкой в центр экрана; фокус остаётся у того окна, которое было активным
- [x] 5.3 Перезапустить демон (`systemctl --user restart workspaced.service`) и убедиться, что привязки перезагрузились сами: число записей в `hyprctl binds` совпадает с числом строк `workspaced keys --list` с поправкой на служебные привязки подкарт (вход в подкарту и `Escape` в ней), а Ctrl+Super+3 и Ctrl+Super+Пробел работают нажатием без ручного `hyprctl reload config-only`
- [x] 5.4 Раскладка: Ctrl+Super+Пробел, Alt+Super+Пробел и Shift+Super+Пробел раскладку не переключают, Super+Пробел переключает; проверка нажатиями в поле ввода, потому что `hyprctl devices` показывает раскладки прежней карты

Проверено 22.09.2026: 3.2 и 5.3 выполнены мной (конфиг применён, демон перезапущен, `hyprctl binds` совпадает с `workspaced keys --list`); перенос, повторный перенос на тот же стол и расстановка проверены командами субагентом, затем пользователем нажатиями (5.1, 5.2); раскладку проверил пользователь: с модификаторами Super+Пробел не переключает (5.4).
