...
- [x] 1.10 `daemon.rs`: вызов `hyprctl reload config-only` вынесен в функцию `reload_hypr_binds` и делается не только после удачного перечитывания конфига, но и один раз при старте демона после удачной загрузки конфига; неудача вызова — предупреждение в журнале, повтора по сроку нет (D15). Причина: после перезапуска демона с новым бинарником композитор не знал о новых привязках — `hyprctl binds` показывал 77 записей против 82 у `workspaced keys --list`. Проверка: `cargo test` (62 теста), `cargo clippy --all-targets -- -D warnings`, `cargo build --release` — чисто

- [x] 1.11 `daemon.rs` и `session.rs`: workspace числится ровно на одном столе (D16). Поднятие с явным столом убирает workspace из списков остальных столов и снимает назначенное ему там ленивое поднятие (`assign_desktop`), выбор следующего workspace стола вынесен в `next_ws`; при чтении снимка сессии списки очищаются от повторов (`dedup_desktops`). Причина: перенос добавлял workspace в новый список, но не убирал из прежнего, повторы копились через снимок, и `next_workspace`, цепочка приложения и ленивое поднятие забирали workspace с другого стола. Проверка: тесты `assign_desktop_keeps_workspace_on_one_desktop`, `next_workspace_does_not_show_the_one_moved_away`, `restored_lists_keep_workspace_on_one_desktop`; `cargo test` (65 тестов), `cargo clippy --all-targets -- -D warnings`, `cargo build --release` — чисто
...
- [x] 4.1 Дельты спецификаций: ws-daemon (новые требования «Перенос workspace на стол» и «Расстановка по команде», «Поднятие workspace на столе» упоминает команду переноса и правило «ровно один стол»), ws-sessions («Восстановление при старте» и «Загрузка сессии» очищают списки столов от повторов), ws-config («Цепочки клавиш и привязки»), hyprland-binds (таблица раскладки и зарезервированные сочетания), hyprland-config (карта XKB), hyprland-session (условие совместимости). Дельты сняты с основных спецификаций уже после архивации `key-cycle-stack`
...
- [ ] 5.1 На столе 2 (`surf`): сдвинуть окно мышью, нажать Ctrl+Super+3 — workspace целиком переезжает на стол 3 вместе со всеми окнами, сдвинутое окно стоит там, куда его сдвинули; Ctrl+Super+2 возвращает `surf` на стол 2; повторное Ctrl+Super+3 на столе 3 ничего не делает, а не забирает workspace соседнего стола. После нескольких переносов подряд `workspaced status --json` показывает каждый workspace ровно на одном столе, а стол, с которого workspace ушёл, — без активного (D16); Super+Tab Tab на таком столе не возвращает ушедший workspace
...
- [ ] 5.3 Перезапустить демон (`systemctl --user restart workspaced.service`) и убедиться, что привязки перезагрузились сами: число записей в `hyprctl binds` совпадает с числом строк `workspaced keys --list` с поправкой на служебные привязки подкарт (вход в подкарту и `Escape` в ней), а Ctrl+Super+3 и Ctrl+Super+Пробел работают нажатием без ручного `hyprctl reload config-only`
- [ ] 5.4 Раскладка: Ctrl+Super+Пробел, Alt+Super+Пробел и Shift+Super+Пробел раскладку не переключают, Super+Пробел переключает; проверка нажатиями в поле ввода, потому что `hyprctl devices` показывает раскладки прежней карты
