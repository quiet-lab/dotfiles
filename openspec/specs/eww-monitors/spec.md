...
Переменная `cpu` MUST вычисляться скриптом `scripts/cpu` как доля занятого времени по дельте `/proc/stat` за 1-секундный сэмпл, в процентах 0–100. Poll — каждые 3 секунды.
...
Каждая шкала MUST быть circular-progress толщиной 10 внутри обёртки 70×70 по центру плитки 103×103, с подписью `${значение}%` в центре. Цвета дуг: CPU $green, RAM $cyan, GPU $magenta; фон дуги rgba(169,177,214,0.15).
...
`scripts/disks` (poll 30s) MUST строить строку на блок из `lsblk -J` (jq → TSV), исключая устройства loop*/zram*/ram* и записи со fstype swap. Метка — LABEL или имя устройства без префикса `/dev`.
