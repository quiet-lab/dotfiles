...
Tile {
    ...
    // WARNING: опрос по таймеру, событий не существует. Погоду отдаёт HTTP-API
    // wttr.in: он отвечает на запрос и об изменении сам не сообщает, подписки нет,
    // узнать новые данные можно только повторным запросом. Опрашивается
    // scripts/weather (сводка и прогноз на 3 дня) раз в 30 минут: прогноз меняется
    // медленнее этого срока, а ходить в чужой сервис чаще незачем. Решение
    // записано в docs/decisions/0007-no-timers.md.
