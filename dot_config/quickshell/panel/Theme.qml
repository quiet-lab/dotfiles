...
QtObject {
    ...
    // Полоса табов плитки активного стола: белый с непрозрачностью 0.05 поверх
    // чёрной заливки плитки — полоса светлее ряда окон ровно на 5 %, больше
    // пользователь не разрешил (23.09.2026). Активный таб не покрыт полосой
    // и остаётся цвета заливки, поэтому сливается с рядом окон под ним.
    readonly property color tabBarBg:   Qt.rgba(1, 1, 1, 0.05)
