...
// края (спецификация qs-notifications), окно подсказки клавиш по центру рабочей области
// (спецификация qs-keys-help) и индикатор цепочки с выходом вверху рабочей области
// (спецификация qs-key-chains). Здесь только окна и композиция колонки;
...
ShellRoot {
    ...
    // Окно подсказки клавиш — третье окно layer-shell на весь экран, карточка по центру
    // рабочей области (экран без полосы панели) на слое
    ...
    Variants {
        ...
        PanelWindow {
            ...
            // Занятая зона — ширина панели, 330 px, без отступа 10 px: по ней
            // композитор центрирует плавающие окна (новые окна, правило center,
            // диспетчер центрирования) и разворачивает окно в режиме maximized,
            // а демон workspaced считает рабочую область (поле reserved монитора).
            // Отступ 10 px команды демона кладут сами, симметрично от всех краёв,
            // поэтому центр рабочей области 330…3840 совпадает с центром 340…3830
            // (изменение work-area-center).
            ...
            // ---- Средняя группа: шкалы, сеть, диски, избранное (680…1277) — по
            // центру промежутка между верхней и нижней группами (140 и 139 px).
            Gauges { x: Theme.margin; y: 680 }
            Network { x: Theme.margin; y: 792 }
            Disks { x: Theme.margin; y: 909 }
            Favorites { x: Theme.margin; y: 1154 }

            // ---- Нижняя группа: столы с уведомлениями, треем и раскладкой (1416…1820),
            // лаунчер (1830…2150). Блок столов — семь одинарных плиток и одна
            // двойная (активный стол), 404 px. В правой колонке 36 px: уведомления
            // на уровне первого стола, под ними трей, внизу раскладка.
            Workspaces { id: workspacesTile; x: Theme.margin; y: 1416 }
            Notifications { x: 294; y: 1416 }
            Tray { x: 294; y: 1462 }
            Lang { x: 294; y: 1784 }
