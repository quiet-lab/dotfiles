// Общие цвета, шрифты и размеры панели и окна ввода пароля (Tokyo Night,
// значения из eww.scss); модуль qs.common, подключается в обе конфигурации
// символической ссылкой common -> ../common.
...
QtObject {
    ...
    // Полоса табов плитки активного стола: тёмно-серый #404040 с непрозрачностью
    // 0.05 поверх чёрной заливки плитки — полоса едва светлее ряда окон
    // (решение пользователя 23.09.2026: не белый, а тёмно-серый, непрозрачность
    // 0.05). Активный таб не покрыт полосой и остаётся цвета заливки, поэтому
    // сливается с рядом окон под ним.
    readonly property color tabBarBg:   Qt.rgba(0x40 / 255, 0x40 / 255, 0x40 / 255, 0.05)
