...
// Оформление общее с панелью: каталог common рядом — символическая ссылка
// на ../common, поэтому Theme и Tile подключаются модулем qs.common, а цвета,
// шрифт, рамка и отступы не копируются. Код панели сюда не подключается:
// её синглтоны импортируют модуль qs собственного корня и в этой
