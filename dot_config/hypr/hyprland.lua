...
-- Win+Пробел по кругу, Win+Alt+E фиксирует US, Win+Alt+R фиксирует RU.
...
hl.config({
    ...
    misc = {
        ...
        -- Настройка общая, правила окон с `focus_on_activate` нет намеренно:
        -- композитор проверяет токен активации только на существование
        -- и не отличает запрос по входящему сообщению от запроса по клику
        -- по уведомлению, поэтому правило для Telegram отняло бы и переход
        -- к окну по клику. Самопроизвольное поднятие окна при сообщении
        -- убрано в самом Telegram настройкой «Подсветка окна»
        -- (изменение telegram-no-focus-steal, см. docs/host-state.md).
...
-- Postman открывается окном 1920×1080 по центру рабочей области: сам он
-- восстанавливает размер прошлого сеанса, а с масштабом интерфейса 1.5 окно
-- выходит слишком большим. Правило center, как и диспетчер центрирования,
-- считает центр по монитору без зоны, которую объявила панель (330 px слева).
hl.window_rule({
...
-- Окно ввода пароля sudo (пространство имён askpass, спецификация qs-askpass):
-- то же правило, что у панели. Окно оформлено как плитка панели, заливка с той
-- же альфой 0.9, поэтому размытие и порог прозрачности совпадают.
hl.layer_rule({
    name         = "askpass-blur",
    match        = { namespace = "^askpass$" },
    blur         = true,
    ignore_alpha = 0.2,
})
