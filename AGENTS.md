...
- Все привязки и цепочки клавиш сессии живут в одном файле `dot_config/workspaced/config.toml`, раздел `[[binds]]` (спецификации hyprland-binds и ws-config). Префикс всех действий с workspace — Super+Tab (после него Super отпускается): `d/w/c` поднять, `1…8` + буква — на столе, `s` сохранить, `Tab` — следующий workspace стола; приложения — цепочки `Super+A` + буква. В `hyprland.lua` собственных `hl.bind` нет: он выполняет код, который печатает `workspaced keys --lua`, а при ошибке загружает копию `~/.local/state/workspaced/keys.lua` и показывает уведомление. Новую привязку добавлять в конфиг демона, проверять `workspaced check` и смотреть `workspaced keys --list`; демон сам вызывает `hyprctl reload config-only` после удачного перечитывания.
