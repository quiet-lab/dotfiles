...
- Состояние рабочих столов и сетку лаунчера поставляет демон `eww-daemon` (подкоманды `workspaces`, `launcher`, `window`, `window-menu`, `hover`, `hover-hide`, `tray-menu`; запуск приложений и фильтр — через сокет: `eww-daemon launch <id>`, `eww-daemon launcher-query <текст>`) (Rust, проект `~/work/pets/eww-daemon`, ссылка `~/.local/bin/eww-daemon`, сборка `cargo build --release`); туда же переезжает остальная логика скриптов. Иконки консольных программ берутся из заголовка окна wezterm `<программа> · …` (`format-window-title` в `dot_config/wezterm/wezterm.lua`).
