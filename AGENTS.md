...
- Правило геометрии: расстояние от края экрана и панели до окна, между окнами и между плитками панели — одно число, 10 px. Рамка окна — 1 px, как рамка плиток панели (`border_size` в `hyprland.lua` и `border.width` в `Tile.qml`). У панели это `Theme.margin` и `Theme.gap`, у демона `gap = 5` при ячейках, отодвинутых на 5 px от границ рабочей области; команды `workspaced half` (Super+Shift+стрелки) и `workspaced maximize` (Super+X, переключатель с возвратом прежней геометрии) считают половины и развёрнутую область по тому же правилу; режим maximized композитора для плавающих окон отступы не учитывает.
