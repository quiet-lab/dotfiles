...
- eww собран из исходников в `~/builds/eww` (симлинк `~/.local/bin/eww`), рабочая ветка `fix-systray-x11-click-coords`: в ней исправлены клики по иконкам трея на X11: экранные координаты, отправка вызова по отпусканию кнопки для элементов без DBusMenu и передача колеса методом Scroll (без этого меню трея через xembedsniproxy не открываются, а колесо не работает). При обновлении eww из upstream ветку нужно перебазировать и пересобрать (`cargo build --release`).
